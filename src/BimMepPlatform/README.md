...
`Core.Ifc` (export IFC4/STEP), `Samples` (programme console démontrant quatre scénarios du cahier des
charges : redimensionnement paramétrique 800×400→1000×500, routage A* avec comparaison de variantes,
résolution automatique d'un conflit gaine/poutre, export IFC4 du modèle), `Tests` (suite xUnit, 49 tests).
...
`dotnet build BimMepPlatform.sln` (0 erreur/0 warning), `dotnet test` (49/49), `dotnet run --project Samples`.
Le fichier `.ifc` généré par `Samples` a en outre été rouvert et parsé par **IfcOpenShell** (Python), avec
génération effective de la géométrie triangulée de chaque `IfcDuctSegment` — une validation par un vrai
moteur IFC, pas seulement une vérification syntaxique. Avec un SDK net9.0 réel, refaire ces commandes avant
